...
from data_utils import geodistance_tensorflow, get_results, latlon2healpix, healpix2latlon, cube, get_values
...
#_PATH = '/home/tconceicao/flickr_sf_300/'
_PATH = '/home/tconceicao/col+flickr_sf/'

LC_PATH = "geo_files/"
_MODEL_FINAL_NAME = '/home/tconceicao/weights/effnet_model_test.h5'
_MODEL_WEIGHTS_FINAL_NAME = '/home/tconceicao/weights/effnet_weights_test.h5'
...
X_train, X_test, Y_train, Y_test = train_test_split(training_images, training_coordinates,
                                    test_size=0.10, random_state = _TRAIN_TEST_SPLIT_SEED)
...
land_coverage = rasterio.open(LC_PATH + 'gm_lc_v3.tif')
land_coverage_data = land_coverage.read(1)

print("Reading elevation data...")
elevation = rasterio.open(LC_PATH + 'gm_el_v1.tif')
elevation_data = elevation.read(1)
print("Reading vegetation coverage data...")
vegetation = rasterio.open(LC_PATH + 'gm_ve_v2.tif')
...
for coordinates in (Y_train + Y_test):
    ...
    aux = get_values(land_coverage, elevation, vegetation, land_coverage_data, elevation_data, vegetation_data,
                     lon, lat)
    #print("Terrain: ", aux[0])
    terrain_labels.append(aux[0])
...
clr = CyclicLR(base_lr=0.00001, max_lr=0.01, step_size=step, mode='triangular2')
