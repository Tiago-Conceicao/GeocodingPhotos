...
from data_utils import geodistance_tensorflow, normalize_values, get_results, latlon2healpix, healpix2latlon, square, cube, get_values
...
#_PATH = '/home/tconceicao/col+flickr_ny/'
#_PATH = '/home/tconceicao/colorized_ny_300_rf35/'
...
def generate_arrays_from_file(X, Y1, Y2, codes_matrix, batchsize):
    while 1:
        line = -1
        ...
        for entry in X:
            if count < batchsize:
                ...
                yield [new_X, np.asarray(codes_matrix * len(new_X))], [new_Y1, new_Y2]
                count = 0
                new_X = np.zeros((batchsize, image_size, image_size, 3))
                new_Y1 = np.zeros((batchsize, 2))
                new_Y2 = np.zeros((batchsize, len(Y2[0])))
...
X_train, X_test, Y_train, Y_test = train_test_split(training_images, training_coordinates,
                                    test_size=t_size, random_state = _TRAIN_TEST_SPLIT_SEED)
